...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, setTempBasal) {
    ...
    if (typeof iob_data.activity === 'undefined' || typeof iob_data.iob === 'undefined' ) {
        rT.error ='Error: iob_data missing somfe property';
        return rT;
    }
	...
	console.error("minDelta is:"+minDelta);
	// detect error -> hmm math.min returns NaN so it cannot convert some value to numbers
	// after some debuuging it comes to  glucose_status.short_avgdelta, glucose_status.long_avgdelta let's set them to 0
	var eerror = ' ';
	if (isNaN(minDelta)){
		
        eerror +='Error:glucose_status.delta is NaN ' +isNaN(glucose_status.delta)+', glucose_status.short_avgdelta is NaN '+isNaN(glucose_status.short_avgdelta)+', glucose_status.long_avgdelta is NaN '+isNaN(glucose_status.long_avgdelta+' ');
        
    }
	if (isNaN(minDelta)){
		eerror += ' minDelta is '+minDelta;
		// setting minDelta to 0
		//minDelta = 0.1;
	} else if (isNaN(minAvgDelta)){
		eerror += 'minAvgDelta is NaN ';
	} else eerror = ' no error ->Strange!';
	//end of eeror detect
	...
	// OK the problem is in deviation (returning NAN) -> minDelta is NaN
	 if (typeof eventualBG === 'undefined'){
		 eerror += "UNDEFINED!";
	 }
    if (typeof eventualBG === 'undefined' || isNaN(eventualBG)) {
        rT.error ='Error: could not calculate eventualBG'+eerror;
        return rT;
    } 
        ...
        if (eventualBG < min_bg) {
            ...
            if (snoozeBG > min_bg) { // if adding back in the bolus contribution BG would be above min
                ...
                if (currenttemp.duration > 15 && (round_basal(basal, profile) === round_basal(currenttemp.rate, profile))) {
                    ...
                    rT.reason += "; setting current basal of " + basal + " as temp";
					/*console.error('basal is '+basal);
					console.error('profile is '+profile);
					console.error('rT is '+rT);
					console.error('currenttemp is '+currenttemp);*/
