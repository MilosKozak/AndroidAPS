...
var round_basal = require('./round-basal');
    ...
    if (typeof(currenttemp) !== 'undefined' && typeof(currenttemp.duration) !== 'undefined' && typeof(currenttemp.rate) !== 'undefined' && currenttemp.duration > 20 && suggestedRate <= currenttemp.rate * 1.2 && suggestedRate >= currenttemp.rate * 0.8) {
        rT.reason += ", but "+currenttemp.duration+"m left and " + round_basal(currenttemp.rate, profile) + " ~ req " + round_basal(suggestedRate, profile) + "U/hr: no action required";
        return rT;
    }
    ...
    if (suggestedRate === profile.current_basal) {
      if (profile.skip_neutral_temps) {
        ...
        reason(rT, 'Setting neutral temp basal of ' + round_basal(profile.current_basal, profile) + 'U/hr');
